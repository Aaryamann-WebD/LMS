...
const TeacherDocument = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/teacher/TeacherDocument/${Data}`, {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/teacher/verification/${Data}`, {
        method: "POST",
        body: formDataObj,
      });
